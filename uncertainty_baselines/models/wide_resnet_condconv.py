...
def _get_routing_weights(inputs,
  ...
  if use_onehot_top_k:
    ...
    print('[TOP_K] top_k values shape = {}'.format(top_values.shape))
    print('[TOP_K] top_k indices shape = {}'.format(top_indices.shape))
    one_hot_tensor = tf.one_hot(top_indices, depth=num_experts)
    print('[TOP_K] one_hot_tensor shape = {}'.format(one_hot_tensor.shape))
    mask = tf.reduce_sum(one_hot_tensor, axis=1)
    print('[TOP_K] Mask shape = {}'.format(mask.shape))
    routing_weights *= mask

  use_softmax_routing = routing_fn in ['softmax', 'noisy_softmax']
  if use_softmax_routing:
    print('[SOFTMAX] routing weights shape = {}'.format(routing_weights.shape))
